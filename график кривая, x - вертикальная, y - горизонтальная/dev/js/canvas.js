document.addEventListener('DOMContentLoaded', () => {
    ...
    const getData = (inputElemens) => {
        ...
        //Метод .map() создает массив, вызывая определенную функцию для каждого элемента родительского массива. Метод .map() не использует мутацию и создает новый массив, в отличие от мутационных методов, которые вносят изменения в вызывающий массив.
    ...
    // Math.PI/2 — перевод 90 градусов в радианы
    ctx.rotate(-Math.PI / 2);
    //точка отсчёта теперь в нижнем левом углу\


    ctx.fillText('x', 50, 10); //x - вертикальная
    ctx.fillText('y', 0, 100); //y - горизонтальная

    ctx.moveTo(0, 0);
    //ctx.quadraticCurveTo(pointX, pointY, x, y); point - точка, в которой искривляется кривая
    //pointx - координата это точки по x


    ctx.quadraticCurveTo(220, 250, 250, 500)
    ctx.stroke();
